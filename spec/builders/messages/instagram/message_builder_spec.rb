...
describe Messages::Instagram::MessageBuilder do
  ...
  context 'when ticketting_interval is set' do
    before do
      instagram_inbox.update(ticketing_interval: 600)
    ...
    it 'creates a new conversation if last conversation is resolved later than ticketting interval' do
      existing_conversation = create(:conversation, account_id: account.id, inbox_id: instagram_inbox.id, contact_id: contact.id,
