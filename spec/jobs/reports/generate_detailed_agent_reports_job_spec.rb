...
RSpec.describe Reports::GenerateDetailedAgentReportsJob do
  ...
  context 'when perform' do
    ...
    it 'generates detailed agent reports for each agent belonging to account' do
      ...
      allow(V2::Reports::Agents::ConversationsCountBuilder).to receive(:new).and_return(report_builder = double)
      allow(report_builder).to receive(:perform).and_return('report')
      ...
      expect(report_builder).to have_received(:perform).once
    end
